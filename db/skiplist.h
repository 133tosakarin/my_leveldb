...
class SkipList  {
 ...
 private:
  ...
  auto Equal(const Key &a, const Key &b) const -> bool { return (compare_(a, b) == 0); }
...
auto SkipList<Key, Comparator>::Contains(const Key &key) const -> bool {
  ...
  if (node != nullptr && Equal(key, node->key)) {
    return true;
  }
  return false;
}
...
void SkipList<Key, Comparator>::Insert(const Key &key) {
  ...
  for (int i = 0; i < new_height; ++i) {
    ...
    // NoBarrier_SetNext() suffices since we will add a barrier
