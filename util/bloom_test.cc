...
constexpr static int kVerbose = 1;
 ...
 public:
  ...
  double FalsePositiveRate() {
    ...
    for (int i = 0; i < 10000; ++i) {
      if (Matches(Key(i + 100000000, buffer))) {
        result++;
      }
    }
...
TEST_F(BloomTest, VaryingLengths) {
  ...
  for (int length = 1; length <= 10000; length = NextLength(length)) {
    ...
    if (rate > 0.0125)
      mediorce_filters++;   // Allowed, but not too often
    else 
      good_filters++;
  }
  if (kVerbose >= 1) {
    std::fprintf(stdout, "Filter: %d good, %d mediorce\n", good_filters,
                  mediorce_filters);
  }
  ASSERT_LE(mediorce_filters, good_filters / 5);
}
