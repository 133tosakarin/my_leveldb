 ...
 public:
  ...
  void Logv(const char *format, std::va_list arguments) override {
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
      ...
      // 3 delimiters) plust the thread ID, which should fit comfortably into the
      // static buffer) 
      ...
      // The code below may append a newline at the end of buffer, which
      // requires an extra character.
      if (buffer_offset >= buffer_size - 1) {
        // The message did not fit into the buffer.
        if (iteration == 0) {
          // Re-run the loop and use a dynamically-allocated  buffer. The buffer
          // will by large enough for the log message, an extra newline and a
          // null terminator.
        ...
        // Fail in tests, recouver by truncating the log message in production.
        assert(false);
        buffer_offset = buffer_size - 1;
      }

      // Add a newline if necessay.
